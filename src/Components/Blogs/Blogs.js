...
const Blogs = () => {
    return (
        <div>
            <p>14.1 How will you improve the performance of a React Application? <br />  1.We can keep the Components state local to improve the perfomance in react
            ...
            <p>14.3 How does prototypical inheritance work? <br /> A prototype is a working object instance. When we read a property from object and it is missing, javascript takes it automatically from the prototype. This is called prototypal inheritance. In order to get and set the Prototype of an object, we use Object, getPrototypeOf.t</p><hr />
            <p>14.4 Why should we not update the state directly?: <br /> We should not update the state directly. Because if we try to set the state directy as a example(products=[]) , then it wont re render the component. We need to use setState (setProducts) because it schedules an update to a component's state object. If the state changes the component then responds by re rendering.
            </p><hr />
